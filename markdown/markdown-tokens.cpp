...
/*
	...
	Modified by Shchipachev Nikita.
*/
...
optional<TokenGroup> RawText::processSpanElements(const LinkIds& idTable) {
	...
	str=_processSpoilerSpans(str, replacements);
	str=_processLink2PostSpans(str, replacements);
...
std::string RawText::_processSpoilerSpans(std::string src, ReplacementTable&
	replacements)
{
	static const boost::regex cSpoilerSpan[2]={
		boost::regex("(?:^|(?<=[^\\\\]))%% (.+?) %%"),
		boost::regex("(?:^|(?<=[^\\\\]))%%(.+?)%%")
	};
	for (int pass=0; pass<2; ++pass) {
		...
		while (1) {
			boost::smatch m;
			if (boost::regex_search(prev, end, m, cSpoilerSpan[pass])) {
				tgt+=std::string(prev, m[0].first);
				tgt+="\x01@"+boost::lexical_cast<std::string>(replacements.size())+"@Spoiler\x01";
				prev=m[0].second;
				replacements.push_back(TokenPtr(new SpoilerSpan(_restoreProcessedItems(m[1], replacements))));
			} else {
				tgt+=std::string(prev, end);
				break;
			}
...
std::string RawText::_processLink2PostSpans(std::string src, ReplacementTable&
	replacements)
{
	...
	for (int pass=0; pass<1; ++pass) {
		...
		while (1) {
			boost::smatch m;
			if (boost::regex_search(prev, end, m, cLink2PostSpan[pass])) {
				tgt+=std::string(prev, m[0].first);
				tgt+="\x01@"+boost::lexical_cast<std::string>(replacements.size())+"@Link2Post\x01";
				prev=m[0].second;
				replacements.push_back(TokenPtr(new Link2PostSpan(_restoreProcessedItems(m[1], replacements))));
			} else {
				tgt+=std::string(prev, end);
				break;
			}
...
void SpoilerSpan::writeAsHtml(std::ostream& out) const {
	out << "<span class=spoiler>";
	TextHolder::writeAsHtml(out);
	out << "</span>";
}

void SpoilerSpan::writeAsOriginal(std::ostream& out) const {
	out << '%' << *text() << '%';
}

void Link2PostSpan::writeAsHtml(std::ostream& out) const {
	out << "<a href=#p";
	TextHolder::writeAsHtml(out);
	out << " >&gt;&gt;";
	TextHolder::writeAsHtml(out);
...
void Link2PostSpan::writeAsOriginal(std::ostream& out) const {
	out << ">>" << *text() << ' ';
}
