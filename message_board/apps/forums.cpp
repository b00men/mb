...
adm_forums::adm_forums(cppcms::service &srv) : master(srv)
{
        mapper().assign("{1}"); // with id
        mapper().assign("");    // default
        dispatcher().assign(".*",&adm_forums::prepare,this,0);
}

void adm_forums::prepare_content(data::adm_forums &c,std::string const &page)
{
        master::prepare(c);
...
void adm_forums::prepare(std::string page)
{       
        ...
        data::adm_forums c;
                ...
                if(c.form.validate()) {
                        ...
                        response().set_redirect_header(url("/adm_thread",tid));
                        c.form.clear();
                        return;
                }
                else {
                ...
                std::string key = "adm_main_page_" + page;
